...
class trans(Ui_control):      # 通信类，每一个设备建立一个对象
    def __init__(self,IP,port,number,machine_name):     # IP地址，端口号，序号，设备名
        ...
        # A路继电器需要发送密码才能继续使用，返回ok（4F 4B）才能通信，若是返回4E 4F说明密码错误

        if self.number == 15:       # 第一台继电器
            state_1 = self.data_write("61 64 6D 69 6E 0D 0A")
            time.sleep(0.1)
            state_2 = self.data_write("61 64 6D 69 6E 0D 0A")       # 二次发送
            state = state_1 + state_2
            if state == 0:          # 说明两次尝试通信都失败了，继电器未开机，或电脑没连接对应wifi，或网线接头没插好
                print("继电器1通信失败")
                self.relay_connect_state_1 = "继电器1通信失败，检查网络和路由设备"
    ...
    def update(self):
        while True:
            ...
            if self.number == 17:     
                ...
                state_1 = self.data_write("55 AA 00 02 00 0A 0C")
                time.sleep(0.1)
                state_2 = self.data_write("55 AA 00 02 00 0A 0C")
                if state_1 + state_2 == 0:      # 如果两次发送指令都失败了
                    self.relay_connect_state_1 = "继电器1通信失败，检查网络和路由设备"
                    print("第1台继电器重连\r" ) 
                    result = self.re_connect()
                    if result == 1:     # 第一台继电器重连成功后还得发送密码
                        time.sleep(0.1)
                        self.data_write("61 64 6D 69 6E 0D 0A")
                        time.sleep(0.1)
                        rec_data = self.data_read()
                        if rec_data == "4F 4B":     # print("A路继电器可通信")
                            print("第1台继电器重连后密码正确，通信成功")
                    self.threadLock.release()
                    continue
                else:
                    print("两次发送指令的成功次数为：%d" % (state_1 + state_2))
                    print("第1台继电器已连接………………………………………………………………………………")
                    time.sleep(0.15)
                ...
                state_1 = self.data_write("FE 01 00 00 00 08 29 C3")
                time.sleep(0.1)
                state_2 = self.data_write("FE 01 00 00 00 08 29 C3")
                if state_1 + state_2 == 0:
                    print("第2台继电器重连\r" ) 
                    self.relay_connect_state_2 = "继电器2通信失败，检查网络和路由设备"
                    self.re_connect()
                    self.threadLock.release()
                    continue
                else:
            ...
            # 电源
                ...
                if state_1+state_2 == 0:
                    # 如果两次发送指令都失败了，尝试重连 
                    print("设备%s尝试重新连接\r" % self.machine_name) 
                    self.re_connect()
                    # 记录此次失败
    ...
    def re_connect(self):
        ...
        try:
            ...
            print("第%d台重连成功！！！\r" % self.number)
            return 1
        except:
            print("第%d台重连失败\r" % self.number)
            return 0
    ...
    def get_ralay(self,order):
        ...
        if order == "000":
            ...
            if self.number == 15:
                self.relay_connect_state_1 = "请检查继电器1是否已开机"
            elif self.number == 16:
                self.relay_connect_state_2 = "请检查继电器2是否已开机 "
        else:
            self.log_data("comm","继电器%s返回指令：%s" % ( self.machine_name,order))
            print("comm","继电器%s返回指令：%s\n" % ( self.machine_name,order))
            ...
            for i in range(0,len(get_v)):      
                if get_v[i:i+2] == "BB" and get_v[i+6:i+20] == "C3 CC 33 C3 3C":
                    ...
                    if get_v[i:i+2] == "BB" and get_v[i+6:i+8] == "C3" and  get_v[i+21:i+32] == "CC 33 C3 3C":
                        ...
                        i += 3
                        self.voltage = "loading"
                # 读到继电器的话继续下一个字节，无需其他处理
                else:
                    i += 3

    # 记录在文件夹下的txt文件，分别按时间和设备类型记录
