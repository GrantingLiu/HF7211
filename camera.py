...
# 创建相机实例并创建句柄,(设置日志路径)
def creat_camera(deviceList , nConnectionNum ,log = False , log_path = getcwd()):
    """
...
# 主动图像采集
def access_get_image(label_width,label_height,cam , active_way = "getImagebuffer"):
    """
    :param cam:     相机实例
    ...
    if active_way == "getImagebuffer":
        ...
        while True:
            ...
            print("已得到 image")
            get_image = QtGui.QImage(image.data, image.shape[1], image.shape[0],QImage.Format_RGB32)  # Format_RGB888，Format_RGB32
            print("设置大小")
            #p = get_image.scaled(320, 240)  # Qt.KeepAspectRatio 保持图片的尺寸
            myWin.label_3.setPixmap(QPixmap.fromImage(get_image).scaled(label_width, label_height))
...
def main():
    ...
    # 主动图像采集
    access_get_image(cam, active_way="getoneframetimeout", nPayloadSize=data_size)
    # 关闭设备与销毁句柄
